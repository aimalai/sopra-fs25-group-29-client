    ...
    const fetchInvites = async () => {
      try {
        ...
        const response = await apiService.get<string[]>(
          `/watchparty/${watchPartyId}/invites`
        );
  ...
  const handleInvite = async (values: { username: string }) => {
    ...
    try {
      ...
      const response = await apiService.post<{ ok: boolean }>(
        `/watchparty/${watchPartyId}/invite?username=${encodeURIComponent(
          values.username
      ...
      if (response.ok) {
        ...
        setSuccessMessage(`🎉 Invite sent successfully to ${values.username}!`); // ✅ Display success message
